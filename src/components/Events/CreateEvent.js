...
const CreateEvent = () => {
  ...
  const handleImageUpload = async (e) => {
    ...
    const storageRef = ref(storage, `events/${id}/${uniqueFileName}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await addDoc(collection(db, 'stories', id, 'events'), eventData);
      alert('Event created successfully!');
      navigate(`/story/${id}/events`); // Navigate back to the events list after creation
    } catch (error) {
